...
export default function ArtistForm({ artist, onSave, onCancel }) {
  ...
  useEffect(() => {
    ...
    if (artist) {
      ...
      setThumbnailPreview(artist.hasThumbnail ? `${ARTISTS_API}/${artist.id}/thumbnail` : null)
    } else {
      setForm(EMPTY_FORM)
  ...
  return (
    <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-4">
      <h2 className="text-xl font-bold text-white">{isEditing ? 'Edit Artist' : 'Add Artist'}</h2>
          ...
          <div className="mb-2 relative inline-block">
            ...
            <button
              ...
              onClick={() => {
                ...
                setThumbnailPreview(null)
                setThumbnailFile(null)
                setClearThumbnail(artist?.hasThumbnail ?? false)
