...
export default function GenreForm({ genre, onSave, onCancel }) {
  ...
  useEffect(() => {
    ...
    if (genre) {
      ...
      setThumbnailPreview(genre.hasThumbnail ? `${GENRES_API}/${genre.id}/thumbnail` : null)
    } else {
      setForm(EMPTY_FORM)
  ...
  return (
    <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-4">
      <h2 className="text-xl font-bold text-white">{isEditing ? 'Edit Genre' : 'Add Genre'}</h2>
          ...
          <div className="mb-2 relative inline-block">
            ...
            <button
              ...
              onClick={() => {
                ...
                setThumbnailPreview(null)
                setThumbnailFile(null)
                setClearThumbnail(genre?.hasThumbnail ?? false)
